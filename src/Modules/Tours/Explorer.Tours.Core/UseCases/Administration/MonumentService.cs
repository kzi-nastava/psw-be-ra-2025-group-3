    ...
    public class MonumentService : IMonumentService
    {
        ...
        public PagedResult<MonumentDto> GetPaged(int page, int pageSize)
        {
            ...
            var items = result.Results
                .Select(_mapper.Map<MonumentDto>)
                .ToList();
        ...
        {
            var created = _monumentRepository.Create(_mapper.Map<Monument>(entity));
            return _mapper.Map<MonumentDto>(created);
        }

        public MonumentDto Update(MonumentDto entity)
        {
            var updated = _monumentRepository.Update(_mapper.Map<Monument>(entity));
            return _mapper.Map<MonumentDto>(updated);
        }
